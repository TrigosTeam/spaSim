...
spaSim is a simulator of tumor immune microenvironment spatial data. It includes a family of functions to simulate a diverse set of cell localization patterns in tissues. Patterns include background cells (one cell type or multiple cell types of different proportions), tumour/immune clusters, immune rings, double immune rings and stripes (blood/lymphatic vessels). 

## Install `spaSim`

`r Biocpkg("spaSim")` is a `R` package available via the [Bioconductor](http://bioconductor.org) repository for packages. `R` can be installed on any operating system from [CRAN](https://cran.r-project.org/) after which you can install `r Biocpkg("spaSim")` by using the following commands in your `R` session:
...
In spaSim, spatial patterns are simulated on separate layers sequentially starting from 'background cells' which serve as the canvas for higher order structures (e.g. tumor clusters, immune clusters and immune rings can be simulated after/on top of background cells). Here we will go through each of these steps.
...
```{r}
mix_bg <- simulate_mixing(bg_sample = bg,                                                                                                                           idents = c("Tumour", "Immune", "Others"),
                         props = c(0.2, 0.3, 0.5), 
                         plot_image = TRUE,
...
The assignment of identities to the cells that lie in the tumour cluster is random, using the random number sampling technique.
...
Similar to simulating the clusters, the assignment of identities to the cells that lie in the tumour cluster and the immune ring is random, using the random number sampling technique.
...
First, we specify the properties of double rings such their primary (inner mass), secondary (inner ring), and tertiary (outer ring) cell types, size, shape, width and location. Properties of cells infiltrating into the inner mass or either ring can also be set. Similar to simulating the clusters, the assignment of identities to the cells that lie in the tumour cluster and the double rings is random, using the random number sampling technique.
...
Sometimes simulations of a set of images that span a range of different properties of patterns are needed. Rather than simulating images individually, simulating these images in one go is desirable. The following functions create a quick interface to generate a range of images with different parameters/randomised elements.
...
This function aims to simulate a set of images that contain different proportions of specified cell types.
                                                                                                        In this example we simulate 4 images with 10% Tumour cells and an increasing number of Immune cells. First specify the cell types and the proportions of each cell type in each image.
...
#But there are 3 elements in the Immune_prop and Others_prop ones
...
Here we simulate 4 images with larger and larger tumour cluster sizes using the cluster shape "1". We also define the range of any desired parameter.

Can you also include examples for shapes "1" and "3" so users know how they look?
```{r}
...
multiple_images_with_clusters(bg_sample = bg1,
                               ...
                               cluster_loc_y = 0,
                               plot_image = TRUE, 
                              plot_categories = c("Tumour" , "Immune"),
                              plot_colours = c("red","darkgreen"))
```
...
multiple_images_with_clusters(bg_sample = bg1,
                               ...
                               cluster_loc_y = 0,
                               plot_image = TRUE, 
                               plot_categories = "Immune",
                               plot_colours = "darkgreen")
```
...
This function aims to simulate a set of images that contain different tumour clusters with immune rings.
                                                                                                         Note that similar to `multiple_images_with_clusters`, in this function users cannot manually define the base shape and the primary cell type of the clusters or the immune rings. There are three options for the base shape available in the `ring`_shape` argument ("1", "2" and "3" where where the primary cluster cell type is "Tumour", cluster infiltration cell types are "Immune" and "Others", primary ring cell type is "Immune" and ring infiltration type is "Others") (better define which correspond to "1", "2" and "3" as it is not clear). The cluster size, infiltration proportions, cluster location, ring width, and ring infiltration proportions can be defined. (Do you mean that you can have a range of these as well?)
