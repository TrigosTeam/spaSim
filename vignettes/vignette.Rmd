...
spaSim has a family of functions to simulate patterns. Suggested text: explain the layered construction of patterns starting from 'background cells' which serve as the canvas for higher order structures. 

Also worth highlighting which parts are deterministic (cluster/shape locations, shape, and size) and which are stochastic (cell assignments within clusters/shapes, vessel locations?), either in this section or the subsections below.

Also worth showing that you can input a background already containing cell assignments into these functions in a later section showing more advanced usage. 

### Simulate background cells.
Suggested text: Randomly generate spatial locations of 'background cells' prior to creating cell identities. The 'background cells' serve as the input to the other simulation functions which may assign new identities to these cells in a structured or unstructured way.
```{r}
...
Suggested text: Randomly assign 'background cells' to the the specified cell identities in the specified proportions in an unstructured manner.
```{r}
...
### Simulate clusters
Suggested text: First, specify the properties of clusters such their primary cell type, size, shape and location, and properties of any infiltrating cell types. Then randomly assign 'background cells' which lie within these clusters to the specified cell identities in the specified proportions.
```{r}
...
### Simulate vessels.
Suggested text: First, specify the properties of vessel structures such as the number present, their width, and the properties of their infiltrating cells. Then randomly assign 'background cells' which lie within these vessel structures to the specified cell identities in the specified proportions.
```{r}
...
Suggested text: explanation of quick interface to generate images with different parameters/randomised elements.

Might be worth using only some of these examples and putting them under a section for advanced usage about generating images in parallel, along with layering multiple shapes, using inputs which already contain assignments.

### Simulate multiple background images with different proportions of cell types.
The example is 4 images with 10% Tumour cells and more and more Immune cells.
```{r}
#Even though it is in the function documentation, I suggest taking the vectors out of the function and declaring them at the start, maybe with comments about what they are when necessary, i.e.:
##cell types present in each image
#cell_types <- c("Tumour", "Immune", "Others")
##each vector i in list corresponds to cell type i. Each element j in vector i corresponds to the proportion of cell type i in image j.
#prop_list <- list(rep(0.1, 4), seq(0, 0.3, 0.1),  seq(0.9, 0.6, -0.1)
bg_list <- 
...
```{r}
#be aware this uses layering of multiple shapes. Best to show after you have introduced this concept.
cluster_list <- 
...
The example is simulating a background sample with a tumour cluster on it.
